...
#define kDefaultRetainPages 1.0 // 1.0: YES, 0.0: NO
...
@interface UIViewController (ICIndexed)
...
@implementation UIViewController (ICIndexed)
...
- (NSUInteger)icIndex {
    NSNumber *number = objc_getAssociatedObject(self, &INDEX_KEY);
    if (number) {
...
- (void)defaultSettings {
    ...
    _retainPages = kDefaultRetainPages;
...
- (void)reloadData {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        ...
        _retainPages = [self.delegate viewPager:self valueForOption:ViewPagerOptionRetainPages withDefault:kDefaultRetainPages];
    }
...
- (UIViewController *)viewControllerAtIndex:(NSUInteger)index {
    ...
    UIViewController *viewController;
    if ([[_contents objectAtIndex:index] isEqual:[NSNull null]] || !_retainPages) {
        ...
        viewController = [_contents objectAtIndex:index];
    }
